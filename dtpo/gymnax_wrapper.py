...
from gymnax.environments.spaces import Space, Discrete, Box, Dict
...
def gymnax_space_to_gymnasium_space(space: Space) -> gymnasium_spaces.Space:
    ...
    if isinstance(space, Discrete):
        ...
        low = (
            float(space.low)
            if (np.isscalar(space.low) or (space.low.size == 1 and space.low.ndim == 0))
            else np.array(space.low)
        )
        high = (
            float(space.high)
            if (
                np.isscalar(space.high)
                or (space.high.size == 1 and space.high.ndim == 0)
            )
            else np.array(space.high)
        )
        return gymnasium_spaces.Box(low, high, space.shape, space.dtype)
    elif isinstance(space, Dict):
        return gymnasium_spaces.Dict(
            {k: gymnax_space_to_gymnasium_space(v) for k, v in space.spaces}
        )
    elif isinstance(space, Tuple):
        return gymnasium_spaces.Tuple(space.spaces)
    else:
...
class GymnaxToGymnasiumWrapper(gymnasium.Env):
    def __init__(
        self,
        env: Environment,
    ...
    def action_space(self):
        ...
        return gymnax_space_to_gymnasium_space(self._env.action_space(self.env_params))

    @property
    def observation_space(self):
        """Dynamically adjust state space depending on params"""
        return gymnax_space_to_gymnasium_space(
...
class GymnaxToVectorGymnasiumWrapper(gymnasium.vector.VectorEnv):
    def __init__(
        self,
        env: Environment,
        num_envs: int = 1,
